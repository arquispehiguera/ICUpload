...
namespace PacificoSeguros.Process.Services
{
    public class WatchdogHostedService : BackgroundService
    {
            ...
            IConfiguration configuration)
        {
            ...
            // hace hasta 4 llamadas secuenciales a la BD por vuelta (InsertMachineOracle,
            // ReclaimOrphanedIniLLamada, PopulateIniLLamada(AGENT), PopulateIniLLamada(MACHINE)
            // — el claim ahora se parte en dos llamados, uno por Origen, para alimentar
            // canales/worker pools separados), cada una envuelta en DbRetry
            ...
            // = 34s. Si las 4 llamadas agotan reintentos en la misma vuelta: ~136s. 270s da
            // casi el doble de margen sobre ese piso sin quedar tan laxo como para no
            // detectar un hang real. Si algún día cambia el CommandTimeout, el delay de
            // DbRetry, o la cantidad de llamadas secuenciales del Producer, este cálculo
            // hay que rehacerlo — no copiar el número a ciegas.
            var minThresholdSeconds = configuration.GetValue<int>("Watchdog:MinStaleThresholdSeconds", 270);
